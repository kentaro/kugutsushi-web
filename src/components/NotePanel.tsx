...
export function NotePanel({ note, onClose }: { note: NoteData; onClose: () => void }) {
  return (
    <div className="absolute inset-0 z-20 flex items-center justify-center p-4 sm:p-8" onClick={onClose}>
      <div className="absolute inset-0 bg-black/60" />
      <article
        className="relative bg-neutral-950 border border-neutral-800 rounded-xl w-full max-w-2xl max-h-[85dvh] overflow-y-auto shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        <div className="sticky top-0 bg-neutral-950/95 backdrop-blur-sm px-6 pt-5 pb-3 border-b border-neutral-800/50 z-10">
          ...
          <h2 className="text-lg font-bold mt-2 leading-snug">{note.title}</h2>
          <div className="flex gap-3 mt-1">
            {note.date && <p className="text-xs text-neutral-500">{note.date}</p>}
